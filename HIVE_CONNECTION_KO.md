...
`HivePlatformExtensionSlot`은 기본적으로 비활성 구현을 가리킨다. 현재 게임은 이
슬롯을 호출하지 않으며 HIVE SDK, 로그인, AppID가 없어도 싱글플레이와 직접 서버
연결이 그대로 동작한다. 후속 버전에서는 `IHivePlatformExtension` 구현 하나를
등록해 인증, 매치메이킹, 업적, 클라우드 저장, 분석, 푸시 알림을 선택적으로 붙일
수 있다. 플랫폼 코드는 이 경계 밖에서 게임의 경제·전투·승패 상태를 직접 바꾸지
않는다.
