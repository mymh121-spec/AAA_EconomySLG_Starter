...
기본 멀티플레이 화면에서 표시 이름으로 최대 4인 방을 바로 만들거나 6자리 초대 코드로 참가한다. 플레이 인원수는 따로 입력하지 않는다. Unity는 생성·참가 응답의 세션 토큰을 `PlayerPrefs`나 파일에 저장하지 않고 현재 프로세스 메모리에만 유지한다. 방장은 참가자가 2명 이상일 때 경기를 시작할 수 있고, 참가자는 `방 상태 갱신`으로 시작된 경기에 진입한다.
